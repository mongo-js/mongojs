...
Collection.prototype.update = function (query, update, opts, cb) {
  ...
  this._getConnection(function (err, connection) {
    ...
    connection.collection(self._fullColName()).update(query, update, opts, function (err, result) {
      if (err) { return cb(err) }
...
Collection.prototype.aggregate = function () {
  ...
  if (cb) {
    self._getConnection(function (err, connection) {
      ...
      connection.collection(self._fullColName()).aggregate(pipeline, cb)
    })
  ...
  var strm = new Cursor(function (cb) {
    self._getConnection(function (err, connection) {
      ...
      cb(null, connection.collection(self._fullColName()).aggregate(pipeline))
    })
  })
