...
var Database = function (connString, cols, options) {
  ...
  if (typeof connString === 'string') {
    ...
    this._getConnection = thunky(function (cb) {
      mongodb.connect(connString, options, function (err, db) {
        if (err) {
          // TODO: Should we really emit error? This requires a listener attached? Only if listeners attached?
        ...
        // TODO: Should we fire a connect event for all other cases too?
        self.emit('connect')
        cb(null, db)
      })
    })
  ...
  // TODO: Should we leave splitting collection names to mongodb?
  cols = cols || []
