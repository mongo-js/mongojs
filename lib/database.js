...
var Database = function (db, options) {
  ...
  if (typeof db.connString === 'string') {
    var config = parse(db.connString)

    this._dbname = config.dbName
    this._server = getTopology(config)

    this._getServer = thunky(function (cb) {
      connect(self._server, config, options, cb)
    })
  } else {
    this._dbname = db.connString._dbname
    this._server = db.connString

    this._getServer = thunky(function (cb) {
      toMongodbCore(db.connString, function (err, server) {
  ...
  db.cols = db.cols || []
  db.cols.forEach(function (colName) {
